...
st.header("2. Données d'Entraînement, Fatigue et Sommeil")
col4, col5, col6 = st.columns(3)

with col4:
    charge_entrainement_ua = st.number_input(
        "Charge d'entraînement hebdomadaire (Unités Arbitraires - UA)",
        min_value=0, max_value=20000, value=5000, step=100,
        help="Calculée comme RPE x durée. Une valeur élevée peut indiquer une charge importante."
    )
...
with col5:
    fatigue_percue = st.slider(
        "Niveau de Fatigue Perçue (échelle 1-10)",
        1, 10, 3,
        help="La fatigue est un prédicteur de blessure. (1: Pas fatigué, 10: Extrêmement fatigué)"
    )
    sommeil_heures = st.slider(
        "Sommeil habituel (heures par nuit)",
        4.0, 10.0, 7.0, step=0.5,
        help="La qualité du sommeil est une donnée de bien-être importante."
    )
...
with col7:
    ...
    force_adducteurs_score = st.slider("Force des Adducteurs (score 1-10)", 1, 10, 7, help="Une réduction de force (ex: <5) est un prédicteur de blessures graves aux membres inférieurs non-contact. (1: Très faible, 10: Très forte)")
    force_generale_newton = st.number_input(
        "Force Musculaire Générale (simulée, en Newtons)",
        min_value=0, max_value=2000, value=500, step=10,
        help="Valeur illustrative. Une force générale faible (ex: <400N) peut augmenter le risque."
    )

with col8:
    force_ischios_score = st.slider("Force des Ischio-jambiers (score 1-10)", 1, 10, 7, help="Une réduction de force (ex: <5) est un prédicteur de blessures graves aux membres inférieurs non-contact. (1: Très faible, 10: Très forte)")
...
with col9:
    force_reaction_sol_score = st.slider(
...
if st.button("Calculer le Risque de Blessure"):
    ...
    # Initialisation des poids de chaque pathologie. Plus le poids est élevé, plus la pathologie est probable
    # en fonction des facteurs de risque
    pathologies_probables_weights = {
    ...
    # --- Évaluation des facteurs de risque et mise à jour du score global et des pathologies ---
    ...
    if hist_blessure_membres_inf:
        ...
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2
    if hist_entorse_cheville:
        ...
        pathologies_probables_weights["Blessures Cheville"] += 3
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 1
    if hist_commotion: #
    ...
    # Charge d'entraînement, fatigue et sommeil
    if monotonie_charge >= 7: #
        ...
        pathologies_probables_weights["Blessures Graves Membres Inférieurs"] += 3
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2

    # Fatigue
    if fatigue_percue >= 7:
        score_risque_global += 18
        facteurs_risque_contributeurs.append(f"Niveau de fatigue perçue élevé ({fatigue_percue}/10)")
        recommandations.append("- Évaluez les stratégies de récupération, y compris le sommeil et la nutrition.")
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2
    if sommeil_heures < 6:
        score_risque_global += 15
        facteurs_risque_contributeurs.append(f"Durée de sommeil insuffisante ({sommeil_heures}h)")
        recommandations.append("- Priorisez la qualité et la quantité de sommeil pour optimiser la récupération.")
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2
    ...
    if sprint_40m > 5.4:
        ...
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2
        pathologies_probables_weights["Blessures Cheville"] += 1
    ...
    if dorsiflexion_cheville < 25:
        ...
        pathologies_probables_weights["Blessures Cheville"] += 3
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2
        pathologies_probables_weights["Blessures Genou (non-ACL)"] += 1 # Peut aussi impacter genou/hanche

    # Forces musculaires
    if force_adducteurs_score < 5:
        score_risque_global += 20
        facteurs_risque_contributeurs.append(f"Force des adducteurs faible ({force_adducteurs_score}/10)")
        recommandations.append("- Renforcez spécifiquement les muscles adducteurs.")
        pathologies_probables_weights["Blessures Aine/Hanche"] += 3
        pathologies_probables_weights["Blessures Graves Membres Inférieurs"] += 2
    if force_ischios_score < 5:
        score_risque_global += 25
        facteurs_risque_contributeurs.append(f"Force des ischio-jambiers faible ({force_ischios_score}/10)")
        recommandations.append("- Priorisez le renforcement excentrique des ischio-jambiers.")
        pathologies_probables_weights["Blessures Ischio-jambiers"] += 4
        pathologies_probables_weights["Blessures Graves Membres Inférieurs"] += 3

    if force_generale_newton < 400: # Seuil arbitraire pour la démo
        score_risque_global += 10
        facteurs_risque_contributeurs.append(f"Force musculaire générale faible ({force_generale_newton}N)")
        recommandations.append("- Évaluez et développez la force musculaire globale.")
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 1
    ...
    if taux_changement_angle_art >= 7:
        ...
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 3
    if force_reaction_sol_score >= 7:
        score_risque_global += 15
        facteurs_risque_contributeurs.append(f"Force de réaction au sol élevée ({force_reaction_sol_score}/10)")
        recommandations.append("- Évaluez et corrigez l'utilisation de la force lors des impacts au sol.")
        pathologies_probables_weights["Blessures Générales Membres Inférieurs"] += 2

    # Poids corporel
    if poids_corporel > 100 and position_joueur == "Avant (Pack)": 
        score_risque_global += 15
        ...
        pathologies_probables_weights["Blessures Cheville"] += 2
    elif poids_corporel > 95 and position_joueur != "Avant (Pack)": 
        score_risque_global += 10
    ...
    if position_joueur == "Avant (Pack)": 
        ...
        pathologies_probables_weights["Blessures Graves Membres Inférieurs"] += 1
    ...
    max_patho_score = max(pathologies_probables_weights.values()) if pathologies_probables_weights else 1
    if max_patho_score == 0: max_patho_score = 1 # Éviter division par zéro

    categories = list(pathologies_probables_weights.keys())
    # Scale pathologies to 0-5. For demonstration, we just show relative "intensity"
    values_pathologies = [ (v / max_patho_score) * 5 for v in pathologies_probables_weights.values()]
    ...
    if fatigue_percue >= 7:
        radar_factors["Fatigue Élevée"] = (fatigue_percue - 5) * 2.5 if fatigue_percue > 5 else 0
    if sommeil_heures < 6:
        radar_factors["Sommeil Insuff."] = 5
    if douleur_musculaire >= 6:
    ...
    if force_generale_newton < 400:
        radar_factors["Force Gén. Faible"] = 5
    if taux_changement_angle_art >= 7:
        radar_factors["Chgmt. Angle Élevé"] = (taux_changement_angle_art - 5) * 2.5 if taux_changement_angle_art > 5 else 0
    if force_reaction_sol_score >= 7:
        radar_factors["GRF Élevée"] = (force_reaction_sol_score - 5) * 2.5 if force_reaction_sol_score > 5 else 0
    ...
    radar_categories_combined = list(pathologies_probables_weights.keys()) + list(radar_factors.keys())
